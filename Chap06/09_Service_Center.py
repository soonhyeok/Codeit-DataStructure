...
class CustomerServiceCenter:
    ...
    def process_complaint(self):
        ...
        if self.queue:
            complain = self.queue.popleft()
            print(f"{complain.name}님의 {complain.content} 문의 내용 접수 되었습니다. 담당자가 배정되면 {complain.email}로 연락드리겠습니다!")
        else:
            print("더 이상 대기 중인 문의가 없습니다!")

    def add_complaint(self, name, email, content):
        """새로운 문의를 큐에 추가 시켜주는 메소드"""
        new_complain = CustomerComplaint(name, email, content)
        self.queue.append(new_complain)
